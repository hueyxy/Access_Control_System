...
class FaceMasker:
    ...
    def get_ref_texture_src(self):
        ...
        for mask_template in mask_template_list:
            ...
            if ref_texture_src.shape[2] == 4:  # 需要一个透明度通道，因为作为一个模板，必须要有这个透明度通道
    ...
    def add_mask_one(self, image_path, face_lms, template_name, masked_face_path):
        ...
        face_mask, new_image = render_cy(np.ascontiguousarray(vertices.T), np.ascontiguousarray(new_colors.T),
                                         np.ascontiguousarray(self.prn.triangles.T.astype(np.int32)), h, w)
        face_mask = np.squeeze(np.floor(face_mask) > 0)  # 在原始图像中 人脸的mask 非人脸区域为0
