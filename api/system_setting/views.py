...
class SystemSettingViewset(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def system_setting_update(self, request, *args, **kwargs):
        ...
        notice_content = SystemSetting.objects.filter(code="temperature_upper").first()
        notice_content.content = serializer.validated_data["temperature_upper"]
        notice_content.save()

        notice_content = SystemSetting.objects.filter(code="temperature_lower").first()
        notice_content.content = serializer.validated_data["temperature_lower"]
        notice_content.save()
    ...
    def get_todo_list(self, request, *args, **kwargs):
        ...
        for todo in todo_list:
            ...
            if todo == "todo[electricity_fees]":
                todo_list_json['electricity_fees'] = ElectricityFees.objects.filter(
                    have_electricity_fees__lte=0).count()
            if todo == "todo[repair]":
            ...
            if todo == "todo[access_control_application]":
                todo_list_json['access_control_application'] = AccessControl.objects.filter(Q(status="abnormal")).count()
            # if todo == "todo[access_control_application]":
            #     todo_list_json['access_control_application'] = AccessControlAbnormalApplication.objects.filter(
            #         Q(result="pending")).count()
    ...
    def get_overview_info(self, request, *args, **kwargs):
        ...
        for info in overview_info:
            if info == "info[student]":
                overview_info_json['student_count_active'] = User.objects.filter(
                    Q(is_staff=False) & Q(is_active=True)).count()
                overview_info_json['student_count_total'] = User.objects.filter(Q(is_staff=False)).count()
            if info == "info[staff]":
                overview_info_json['staff_count_active'] = User.objects.filter(
                    Q(is_staff=True) & Q(is_active=True)).count()
            ...
            if info == "info[dormitory]":
                ...
                overview_info_json['dormitory_count_empty'] = Dormitory.objects.filter(
                    ~Q(allow_live_number__icontains=F("now_live_number"))).count()
            if info == "info[access_control_day]":
                overview_info_json['access_control_day'] = AccessControl.objects.filter(
                    Q(add_time__gte=datetime.now().date())).count()
            ...
            if info == "info[access_control_week]":
                ...
                # 计算当前日期所在周一零点零分零秒 monday = datetime.now() - timedelta(days=day_num) - timedelta(hours=datetime.now(
                # ).hour, minutes=datetime.now().minute, seconds=datetime.now().second, microseconds=datetime.now(
                # ).microsecond)
                monday = datetime.strptime(datetime.now().strftime("%Y-%m-%d"), "%Y-%m-%d")
                overview_info_json['access_control_week'] = AccessControl.objects.filter(
                    Q(add_time__range=(monday, datetime.now()))).count()
                overview_info_json['access_control_total'] = AccessControl.objects.filter().count()
            if info == "info[access_control_month]":
                overview_info_json['access_control_month'] = AccessControl.objects.filter(
                    Q(add_time__month=datetime.now().month)).count()
                overview_info_json['access_control_total'] = AccessControl.objects.filter().count()
    ...
    def data_overview_data(self, request, *args, **kwargs):
        ...
        data = []
        while start_date.__le__(datetime.strptime(datetime.now().strftime("%Y-%m-%d"), "%Y-%m-%d")):
            data.append(AccessControl.objects.filter(
                add_time__range=(start_date.date(), (start_date + timedelta(days=1)))).count())
            start_date += timedelta(days=1)
