...
from .serializers import AccessControlSerializer, AccessControlUpdateSerializer, \
    AccessControlAbnormalApplicationSerializer, AccessControlAbnormalApplicationUpdateSerializer, \
    AccessControlAbnormalApplicationReplySerializer
...
class AccessControlViewset(mixins.RetrieveModelMixin, mixins.ListModelMixin, mixins.UpdateModelMixin,
    ...
    def create(self, request, *args, **kwargs):
        """
            门禁记录 创建
            url: '/access_control/?user_id=<user_id>&euclidean_distance=<euclidean_distance>&temperature=<temperature>'
            type: 'post'
            dataType: 'json'
            data: file
        """
        ...
        if float(request.GET.get('euclidean_distance', '')) > 0.8:
            accuracy = 100
        else:
            accuracy = (0.2 + float(request.GET.get('euclidean_distance', ''))) * 100
        temperature = float(request.GET.get('temperature', ''))
        if float(SystemSetting.objects.filter(code="temperature_upper").first().content) > temperature > float(
                SystemSetting.objects.filter(code="temperature_lower").first().content):
            access_control_status_t = "normal"
            state = "操作成功：1分钟内此人已通过门禁，已自动放行。"
        else:
            access_control_status_t = 'abnormal'
            state = "体温异常，不要放行。"

        access_control_list = AccessControl.objects.filter(person_id=request.GET.get('user_id', '')).order_by(
            "-add_time")

        if access_control_list:
            last_access_control = access_control_list[0]
            five_minutes_ago = datetime.now() - timedelta(hours=0, minutes=0, seconds=30)
        ...
        if datetime.strptime(SystemSetting.objects.filter(code="normal_access_start_time").first().content,
                             "%H:%M:%S").time() < now_time < datetime.strptime(
            SystemSetting.objects.filter(code="normal_access_end_time").first().content, "%H:%M:%S").time():
            access_control_status = "normal"
        elif access_control_status_t == "abnormal":
            access_control_status = "abnormal"
        else:
    ...
    def abnormal_application_create(self, request, *args, **kwargs):
        ...
        system_log = SystemLog.objects.create(
            content='创建门禁异常申请（申请编号：' + str(access_control_abnormal_application.id) + '）',
            category="门禁管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
        system_log.save()
    ...
    def abnormal_application_reply(self, request, *args, **kwargs):
        ...
        system_log = SystemLog.objects.create(
            content='门禁异常申请回复（申请编号：' + str(instance.abnormal_application.id) + '）',
            category="门禁管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
        system_log.save()
