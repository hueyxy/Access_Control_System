...
class AccessControl(models.Model):
    ...
    person = models.ForeignKey(User, verbose_name="通过的人", on_delete=models.CASCADE,
                               related_name='access_control_s_person')
    accuracy = models.DecimalField(max_digits=5, decimal_places=2, verbose_name="准确率(%)")
    temperature = models.DecimalField(max_digits=4, decimal_places=2, verbose_name="体温(摄氏度)")
    status = models.CharField(verbose_name="状态", max_length=8,
                              choices=(("normal", "识别正常"), ("later", "学生晚归"), ("abnormal", "识别异常")),
                              default="normal")
...
class AccessControlAbnormalApplication(models.Model):
    ...
    门禁记录 识别异常申请
    """
    main_record = models.OneToOneField(AccessControl, verbose_name="主记录", on_delete=models.CASCADE,
                                       related_name='abnormal_application')
    content = models.TextField(verbose_name="申请内容")
    result = models.CharField(verbose_name="结果", max_length=7,
                              choices=(("pending", "待处理"), ("pass", "通过"), ("fail", "未通过")), default="pending")
