...
from .serializers import SecurityCheckOldEmailSerializer, SecurityConfirmOldEmailSerializer, \
    SecurityChangeMobileSerializer, SecuritySendNewEmailCaptchaSerializer
from .serializers import SecurityChangeEmailSerializer, SecurityChangePasswordSerializer
from .serializers import SecurityCheckAccountPasswordSerializer, SecuritySendBindEmailCaptchaSerializer, \
    SecurityBindEmailSerializer
...
class UserViewset(mixins.RetrieveModelMixin, mixins.ListModelMixin, mixins.UpdateModelMixin, mixins.CreateModelMixin,
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.user.is_superuser is False:
            ...
            all_result = all_result.filter(Q(is_superuser=False))
        # 分页页数
    ...
    def create(self, request, *args, **kwargs):
        ...
        system_log = SystemLog.objects.create(
            content='创建单个用户（用户名：' + serializer.validated_data["username"] + '）',
            category="用户管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
        system_log.save()
    ...
    def create_multiple(self, request, *args, **kwargs):
        ...
        system_log = SystemLog.objects.create(
            content='创建批量用户（首用户名：' + str(serializer.validated_data["first_username"]) + '，创建数量：' + str(
                serializer.validated_data["create_number"]) + '）',
            category="用户管理",
            operator=request.user,
            ip=request.META.get('REMOTE_ADDR'))
        system_log.save()
    ...
    def reset_password_multiple(self, request, *args, **kwargs):
        ...
        for i in ids:
            ...
            user.check_password(serializer.validated_data["password"], user.password)
            # user.set_password(serializer.validated_data["password"])
            # user.save()
            user.delete()

        # system_log = SystemLog.objects.create(
        #     content='批量重置用户密码（用户名组：' + serializer.validated_data["ids"] + '）',
        #     category="用户管理",
        #     operator=request.user,
        #     ip=request.META.get("REMOTE_ADDR"))
        # system_log.save()
        system_log = SystemLog.objects.create(
            content='删除指定用户（用户名组：' + serializer.validated_data["ids"] + '）',
            category="用户管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
        system_log.save()
    ...
    def set_face(self, request, *args, **kwargs):
        ...
        id2chiclass = {0: '口罩', 1: '无戴口罩'}
        image_path = settings.MEDIA_ROOT.replace("\\", "/") + "/" + flag
        face_512d_features, class_mask = face_recognition.return_face_128d_features(image_path)
        if type(face_512d_features) != numpy.ndarray:
            if face_512d_features == 0:
                os.remove(image_path)
                return Response({
                    "detail": "操作失败：未检测到人脸，请尝试将人脸置于正中！"
                }, status=status.HTTP_400_BAD_REQUEST)

            if face_512d_features == 1:
                os.remove(image_path)
                return Response({
        ...
        # print(type(face_128d_features.tolist()))
        instance = self.get_object()
        if hasattr(instance, 'face'):
            ...
            if class_mask:
                instance.face.photo = flag
                instance.face.features = face_512d_features.tolist()
            else:
                instance.face.features_mask = face_512d_features.tolist()
            ...
            if class_mask:
                face = UserFace.objects.create(photo=flag,
                                               # features=json.dumps(face_512d_features_list),
                                               features=face_512d_features.tolist(),
                                               user=instance)
            else:
                face = UserFace.objects.create(photo=flag,
                                               # features=json.dumps(face_512d_features_list),
                                               features_mask=face_512d_features.tolist(),
                                               user=instance)

            face.save()

        system_log = SystemLog.objects.create(
            content='设置' + id2chiclass[class_mask] + '人脸（用户名：' + instance.username + '）',
            category="用户管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
        system_log.save()
        ...
        return Response({
            "code": 0,
            "msg": "操作成功：人脸数据设置成功！" + id2chiclass[class_mask],
            "data": {
                "src": "http://" + request.META['HTTP_HOST'] + "/media/" + flag
            }
        }, status=status.HTTP_200_OK)
...
class SecurityViewset(viewsets.GenericViewSet):
    ...
    def send_old_email_captcha(self, request, *args, **kwargs):
        ...
        captcha_list = CaptchaModel.objects.filter(email=serializer.validated_data['old_email']).order_by(
            "-create_time")
        if captcha_list:
    ...
    def change_mobile(self, request, *args, **kwargs):
        ...
        system_log = SystemLog.objects.create(
            content='修改手机（用户名：' + request.user.username + '，新手机：' + serializer.validated_data[
                "new_mobile"] + '）',
            category="用户管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
    ...
    def send_new_email_captcha(self, request, *args, **kwargs):
        ...
        captcha_list = CaptchaModel.objects.filter(email=serializer.validated_data['new_email']).order_by(
            "-create_time")
        if captcha_list:
            if captcha_list[0].create_time >= datetime.now() - timedelta(hours=0, minutes=1, seconds=0):
                return Response({
    ...
    def change_email(self, request, *args, **kwargs):
        ...
        system_log = SystemLog.objects.create(
            content='修改邮箱（用户名：' + request.user.username + '，新邮箱：' + serializer.validated_data[
                'new_email'] + '）',
            category="用户管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
    ...
    def send_bind_email_captcha(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        captcha_list = CaptchaModel.objects.filter(email=serializer.validated_data['bind_email']).order_by(
            "-create_time")
        if captcha_list:
            if captcha_list[0].create_time >= datetime.now() - timedelta(hours=0, minutes=1, seconds=0):
                return Response({
    ...
    def bind_email(self, request, *args, **kwargs):
        ...
        system_log = SystemLog.objects.create(
            content='绑定邮箱（用户名：' + request.user.username + '，新邮箱：' + serializer.validated_data[
                'bind_email'] + '）',
            category="用户管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
...
class QQLoginViewset(viewsets.GenericViewSet):
    ...
    def login(self, request, *args, **kwargs):
        ...
        if code == '' or state == '':
            return redirect('http://118.178.89.223:11455/views/qqlogin.html?error_code=99999')
        if state == 'bind':
            return redirect('http://118.178.89.223:11455/views/qqlogin.html?code=' + code)

        token_result = requests.get(
            url='https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=' + settings.QQCONNECT_ID + '&client_secret=' + settings.QQCONNECT_KEY + '&code=' + code + '&redirect_uri=http://hl.acgxt.com:11453/user/login_qq/login?code=' + code + '&state=' + state)
        if 'callback' in token_result.text:
            token_result_json = json.loads(token_result.text.replace('callback( ', '').replace(' );', ''))
            return redirect('http://118.178.89.223:11455/views/qqlogin.html?error_code=' + token_result_json['error'])
        else:
            token_result_json = {}
        ...
        me_result = requests.get(
            url='https://graph.qq.com/oauth2.0/me?access_token=' + token_result_json['access_token'])
        me_result_json = json.loads(me_result.text.replace('callback( ', '').replace(' );', ''))
        if me_result_json['client_id'] != settings.QQCONNECT_ID:
            return redirect('http://118.178.89.223:11455/views/qqlogin.html?error_code=99998')
        ...
        if state == 'login':
            if QQLoginTokenModel.objects.filter(openid=me_result_json['openid']):
                ...
                return redirect('http://118.178.89.223:11455/views/qqlogin.html?token=' + token)
            else:
                return redirect('http://118.178.89.223:11455/views/qqlogin.html?error_code=99997')
    ...
    def bind(self, request, *args, **kwargs):
        ...
        token_result = requests.get(
            url='https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=' + settings.QQCONNECT_ID + '&client_secret=' + settings.QQCONNECT_KEY + '&code=' + code + '&redirect_uri=http://hl.acgxt.com:11453/user/login_qq/login?code=' + code + '&state=bind')
        if 'callback' in token_result.text:
            token_result_json = json.loads(token_result.text.replace('callback( ', '').replace(' );', ''))
            return Response({
    ...
    def get_bind_info(self, request, *args, **kwargs):
        ...
        if QQLoginTokenModel.objects.filter(user=request.user):
            ...
            get_user_info_result = requests.get(
                url='https://graph.qq.com/user/get_user_info?access_token=' + qq_login_token.access_token + '&oauth_consumer_key=' + settings.QQCONNECT_ID + '&openid=' + qq_login_token.openid)
