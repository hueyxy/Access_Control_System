...
class UserUpdateSerializer(serializers.ModelSerializer):
    ...
    info__birthday = serializers.DateField(source='info.birthday', help_text="出生年月", allow_null=True,
                                           required=False)
    info__gender = serializers.ChoiceField(source='info.gender', help_text="性别",
                                           choices=(("male", "男"), ("female", "女"), ("unknown", "未知")))
    info__mobile = serializers.CharField(source='info.mobile', help_text="电话", required=False, allow_blank=True,
                                         max_length=11)
    ...
    def validate_info__mobile(self, mobile):
        if len(mobile) == 0:
            return mobile
        flag = re.match(r'(^$)|^1(3[0-9]|4[5,7]|5[0,1,2,3,5,6,7,8,9]|6[2,5,6,7]|7[0,1,7,8]|8[0-9]|9[1,8,9])\d{8}$',
                        mobile)
        if flag is None:
...
class UserFaceListSerializer(serializers.ModelSerializer):
    ...
    face__add_time = serializers.DateTimeField(source='face.add_time', help_text="账户创建时间",
                                               format="%Y-%m-%d %H:%M")
